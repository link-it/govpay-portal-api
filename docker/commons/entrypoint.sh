...
##############################################################################
# GovPay Portal API - Script di Entrypoint
#
...
log_info "========================================"
log_info "Avvio Servizio GovPay Portal API"
log_info "========================================"
...
GOVPAY_PORTAL_MIN_POOL=${GOVPAY_PORTAL_MIN_POOL:-2}
GOVPAY_PORTAL_MAX_POOL=${GOVPAY_PORTAL_MAX_POOL:-5}
    ...
    export SPRING_DATASOURCE_HIKARI_MINIMUM_IDLE="${GOVPAY_PORTAL_MIN_POOL}"
    export SPRING_DATASOURCE_HIKARI_MAXIMUM_POOL_SIZE="${GOVPAY_PORTAL_MAX_POOL}"
...
JVM_MEMORY_OPTS="-XX:MaxRAMPercentage=${GOVPAY_PORTAL_JVM_MAX_RAM_PERCENTAGE:-${DEFAULT_MAX_RAM_PERCENTAGE}}"
[ -n "${GOVPAY_PORTAL_JVM_INITIAL_RAM_PERCENTAGE}" ] && JVM_MEMORY_OPTS="$JVM_MEMORY_OPTS -XX:InitialRAMPercentage=${GOVPAY_PORTAL_JVM_INITIAL_RAM_PERCENTAGE}"
[ -n "${GOVPAY_PORTAL_JVM_MIN_RAM_PERCENTAGE}" ] && JVM_MEMORY_OPTS="$JVM_MEMORY_OPTS -XX:MinRAMPercentage=${GOVPAY_PORTAL_JVM_MIN_RAM_PERCENTAGE}"
[ -n "${GOVPAY_PORTAL_JVM_MAX_METASPACE_SIZE}" ] && JVM_MEMORY_OPTS="$JVM_MEMORY_OPTS -XX:MaxMetaspaceSize=${GOVPAY_PORTAL_JVM_MAX_METASPACE_SIZE}"
[ -n "${GOVPAY_PORTAL_JVM_MAX_DIRECT_MEMORY_SIZE}" ] && JVM_MEMORY_OPTS="$JVM_MEMORY_OPTS -XX:MaxDirectMemorySize=${GOVPAY_PORTAL_JVM_MAX_DIRECT_MEMORY_SIZE}"
...
log_info "Java: MaxRAMPercentage=${GOVPAY_PORTAL_JVM_MAX_RAM_PERCENTAGE:-${DEFAULT_MAX_RAM_PERCENTAGE}}%"
log_info "========================================"
...
JAR_FILE=$(find /opt/govpay-portal -name "*.jar" -type f | head -n 1)

if [ -z "${JAR_FILE}" ]; then
    log_error "Nessun file JAR trovato in /opt/govpay-portal"
    exit 1
fi
