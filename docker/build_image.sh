...
Sorgente:
-v <VERSIONE>  : Imposta la versione della release binaria da utilizzare per il build (default: ${LATEST_GOVPAY_PORTAL_RELEASE})
...
LATEST_LINK="$(curl -qw '%{redirect_url}\n' https://github.com/link-it/govpay-portal-api/releases/latest 2> /dev/null)"
...
if [ -z "$TAG" ]
then
    REPO=${REGISTRY_PREFIX}/govpay-portal-api
